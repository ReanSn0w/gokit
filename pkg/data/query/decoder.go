...
// decode декодирует url.Values в структуру data. Аргумент data должен быть
// непустым указателем на struct, иначе возвращается ошибка.
//
// Для каждого экспортируемого поля структуры имя query-параметра берётся из
// тега `query`; если тег отсутствует — используется имя поля. Приватные поля
// (которые нельзя установить через reflect) пропускаются.
//
// После заполнения всех полей вызывается [base.Validate](data). Если
// валидация не прошла — возвращается соответствующая ошибка.
//
// Возможные ошибки:
//   - data не является непустым указателем на struct;
//   - тип поля или элемента среза не поддерживается (см. [setFieldValue]);
//   - валидация через base.Validate вернула ошибку.
...
// setFieldValue устанавливает значение одного поля структуры из среза строк
// values. Для скалярных типов используется values[0]; для срезов заполняются
// все элементы values.
//
// Поддерживаемые типы (и их срезы):
//   - string
...
//   - bool
//
// Для любого другого типа возвращается ошибка вида
// «неподдерживаемый тип поля: <kind>» или
// «неподдерживаемый тип элемента среза: <kind>».
